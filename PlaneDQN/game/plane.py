...
class GameState:
    ...
    def frame_step(self, input_actions):
        ...
        # 绘制背景
        if self.display:self.screen.fill(0)
        # screen.blit(background, (0, 0))

        # 绘制玩家飞机
        if not self.player.is_hit:
            if self.display:self.screen.blit(self.player.image[0], self.player.rect) #将正常飞机画出来
        else:
            # 玩家飞机被击中后的效果处理
            if self.display:self.screen.blit(self.player.image[1], self.player.rect) #将爆炸的飞机画出来

        # 敌机被子弹击中效果显示
        for enemy_down in self.enemies_down:
            self.enemies_down.remove(enemy_down)
            self.score += 1
            if self.display:self.screen.blit(enemy_down.down_imgs, enemy_down.rect) #将爆炸的敌机画出来
            reward = 1

        # 显示子弹
        if self.display:self.player.bullets.draw(self.screen)
        # 显示敌机
        if self.display:self.enemies1.draw(self.screen)
