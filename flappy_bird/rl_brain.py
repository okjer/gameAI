...
class DeepQNetwork:
    ...
    def reset(self, observation):
        self.current_state = np.stack((observation, observation, observation, observation), axis=2)# shape 80 80 4 把初始图叠成4张
    ...
    def choose_action(self, observation):
        ...
        if random.random() <= self.epsilon:
            ...
            # 只输入了一个状态
            q_value = self.q_value.eval(feed_dict={self.state_input: [self.current_state]})[0] #根据Qlayer算出每个actiond的q_value
    ...
    def learn(self):
        ...
        if self.time_step % 10000 == 0:
            self.saver.save(self.session, 'mysaved_networks/network-dqn', global_step=self.time_step)
    ...
    def _create_network(self):
        ...
        checkpoint = tf.train.get_checkpoint_state("mysaved_networks")
